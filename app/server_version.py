...
TMP = os.path.join(ROOT, "../enrichmentApp/tmp")
...
# relative imports
sys.path.append(os.path.join(ROOT, "../enrichmentApp/src/"))
...
# set page layout
st.set_page_config(layout="wide", page_title="Protein Set Enrichment Analysis")
...
st.sidebar.title("Proteome Set Enrichment Analysis")
...
the_screening_fids = []
with open(os.path.join(DATA, "the_screening_fids.csv"), "r") as f:
    reader = csv.reader(f)
    for r in reader:
        the_screening_fids += [r[0]]
the_screening_fids = set(the_screening_fids)
...
if profile_type == "Global":
    ...
    for fid in listdir_util(os.path.join(DATA, "signatures", "proteins", "fragment")):
        fids += [fid]
    fids = sorted(fids)
    fids = [fid for fid in fids if fid in the_screening_fids]
