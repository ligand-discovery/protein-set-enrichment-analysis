...
def robust_loess1d(x, yl, frac):
    for f in np.linspace(frac, 1, 100):
        try:
            xout, yout, wout = loess_1d(x, yl, frac=f)
            return xout, yout, wout
        except:
            xout, yout, wout = None, None, None
    return xout, yout, wout


def loess_interpolation(x, y, frac=0.5):
    yl = np.array(y)
    xout, yout, wout = robust_loess1d(x, yl, frac=frac)
    return interpolate.interp1d(xout, yout, fill_value="extrapolate")  # Modified
...
def estimate_parameters(
    ...
    set_sizes = pd.DataFrame(
        list(cc.items()), columns=["set_size", "count"]
    ...
    b = min(int(len(signature) * 0.6), 4000)
    a = int(b / 2)
    x = sorted(list(set(np.append([1, 4, 6, np.max(ll), a, b], nn).astype("int"))))
    ...
    with multiprocessing.Pool(jobs) as pool:
        ...
        results = list(
            tqdm(
                pool.imap(estimate_anchor_star, args),
                desc="Calibration",
                total=len(args),
            )
        )
    ...
    for res in results:
        f_alpha_pos, f_beta_pos, f_ks_pos, f_alpha_neg, f_beta_neg, f_ks_neg, f_pos_ratio = (
            res
        )
    ...
    if np.max(pos_ratio) > 1.5 and verbose:
        print(
            "Significant unbalance between positive and negative enrichment scores detected. Signature values are not centered close to 0."
        )
...
def estimate_anchor(
    signature, abs_signature, signature_map, set_size, permutations, symmetric, seed
):
    es = np.array(
        get_peak_size(
            signature, abs_signature, signature_map, set_size, permutations, seed
        )
    )
...
def gsea(
    ...
    if permutations < 1000 and not symmetric:
        ...
        if verbose:
            print(
                "Low numer of permutations can lead to inaccurate p-value estimation. Consider increasing number of permutations."
            )
        symmetric = True
    ...
    if sig_hash in blitzgsea_signature_anchors.keys() and signature_cache:
        ...
        f_alpha_pos, f_beta_pos, f_pos_ratio, ks_pos, ks_neg = blitzgsea_signature_anchors[
            sig_hash
        ]
    else:
        f_alpha_pos, f_beta_pos, f_pos_ratio, ks_pos, ks_neg = estimate_parameters(
            signature,
            abs_signature,
            signature_map,
            library,
        ...
        blitzgsea_signature_anchors[sig_hash] = (
            f_alpha_pos,
            f_beta_pos,
            f_pos_ratio,
            ks_pos,
    ...
    for k in tqdm(keys, desc="Enrichment "):
        ...
        if len(stripped_set) >= min_size and len(stripped_set) <= max_size:
            ...
            if es > 0:
                ...
                prob_two_tailed = np.nanmin(
                    [0.5, (1 - np.min([(1 - pos_ratio) + prob * pos_ratio, 1]))]
                )
                ...
                if prob > 0.999999:
                    prob = gammacdf(-es, float(pos_alpha), float(pos_beta))
                prob_two_tailed = np.nanmin(
                    [0.5, (1 - np.min([prob * (1 - pos_ratio) + pos_ratio, 1]))]
                )
    ...
    fdr_values = multipletests(np.array(pvals) + 1e-18, method="fdr_bh")[1]
    sidak_values = multipletests(np.array(pvals) + 1e-18, method="sidak")[1]

    res = pd.DataFrame(
        [
            gsets,
            np.array(ess).astype("float"),
            np.array(ness).astype("float"),
            np.array(pvals).astype("float"),
            np.array(sidak_values).astype("float"),
            np.array(fdr_values).astype("float"),
            np.array(set_size).astype("int"),
            np.array(legeness),
        ]
    ).T
    res.columns = [
