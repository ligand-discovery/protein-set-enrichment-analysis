...
def running_sum(signature, geneset, library, result=None, compact=False):
    ...
    if compact:
        ...
        ax1.vlines(
            x=zero_cross, ymin=np.min(rank_vec), ymax=np.max(rank_vec), linestyle=":"
        )
        ax1.text(
            zero_cross,
            np.max(rank_vec) / 3,
            "Zero crosses at " + str(zero_cross),
...
def top_table(signature, library, result, n=10):
    ...
    for i in range(n):
        ax.text(
            0.03,
            (ln[i] + ln[i + 1]) / 2,
            "{:.3f}".format(result.iloc[i, 1]),
            verticalalignment="center",
        )
        ax.text(
            0.84, (ln[i] + ln[i + 1]) / 2, result.index[i], verticalalignment="center"
        )
